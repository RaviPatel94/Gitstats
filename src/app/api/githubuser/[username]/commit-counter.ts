...
export const getAccurateCommitCount = async (
  ...
  try {
    ...
    if (timelineResponse.data?.data?.user) {
      ...
      const totalReposCount = userData.repositories.totalCount;
      const totalPRs = userData.pullRequests.totalCount;
      const totalIssues = userData.openIssues.totalCount + userData.closedIssues.totalCount;
      ...
      return {
        ...
        method: `contribution-timeline-public-${yearsRange.length}years+${estimatedInitialCommits}initial`,
      };
    } else {
...
export const getComprehensiveCommitCount = async (
  ...
  const result = await getAccurateCommitCount(username, token, userData);
