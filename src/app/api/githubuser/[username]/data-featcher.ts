import type { UserStatsData, UserStatsResult, GraphQLResponse } from "./types";
...
import { MissingParameterError } from "./types";
...
export const getUserStats = async (
  ...
  const apiResponse = await retryApiCall(
    (vars, tkn) => getCompleteUserData(vars.login, tkn),
    { login: username },
    token,
  );
  ...
  const commitsResult = await getAccurateCommitCount(username, token, userData);
...
export const getUserTopLanguages = async (userData: UserStatsData, excludeRepos: string[] = []): Promise<string[]> => {
  ...
  repositoryNodes = repositoryNodes
    .filter((repo) => !hiddenReposMap[repo.name])
